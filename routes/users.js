...
router.get('/cart', verifyLogin, async (req, res, next) => {
  try {
    ...
    res.render('users/cart', { layout: 'user-layout', users: true, user, products, category, cartCount, total, user2, banner })
  } catch (err) {
    next(err)
  }
})
...
router.get('/checkout', verifyLogin, async (req, res, next) => {
  try {
    ...
    let banner = await adminHelpers.viewBanner()
    res.render('users/checkout', { users: true, layout: 'user-layout', user, products, total, userAddress, banner })
  } catch (err) {
    next(err)
  }
})
